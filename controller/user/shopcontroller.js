...
//     // Attach active offer if exists
//     const now = new Date();
//     for (let product of products) {
//       const activeOffer = await ProductOffer.findOne({
//         product: product._id,
...
//       if (activeOffer) {
//         product.offer = activeOffer; // add offer info to product
//       }
//     }
...
//     // ✔ First available size index
//     let firstIndex = product.quantities.findIndex(q => q > 0);
//     if (firstIndex === -1) firstIndex = 0;

//     // ✔ Base original price
//     let originalPrice = product.prices[firstIndex] || 0;

//     // ✔ Offer price
//     let offerPrice = originalPrice;
//     if (offer) {
//       offerPrice = Math.floor(originalPrice - (originalPrice * offer.offerPercentage / 100));
//     }
...
//     let originalPrice = product.prices && product.prices.length > 0 
//       ? product.prices[0] 
//       : 0;
...
//     if (offer) {
//       offerPrice = originalPrice - (originalPrice * offer.offerPercentage / 100);
...
//     if (quantity > 8) return res.json({ success: false, message: "Cannot add more than 8 items per product!" });
...
//     if (existingIndex > -1) {
//       if (cart.products[existingIndex].quantity + quantity > availableQty) {
//         return res.json({ success: false, message: `Only ${availableQty} items available` });
//       }
//       cart.products[existingIndex].quantity += quantity;
//       cart.products[existingIndex].pricePerUnit = unitPrice;
//     } else {
//       cart.products.push({
...
//       (acc, p) => acc + ((p.quantity || 0) * (p.pricePerUnit || 0)),
//       0
//     );
//     cart.subtotal = subtotal;
//     cart.grandTotal = subtotal + (cart.shippingCost || 0);
